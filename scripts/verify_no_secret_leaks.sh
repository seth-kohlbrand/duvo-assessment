...
docker run --rm --entrypoint sh -v "$PWD:/workspace:ro" -w /workspace "$image_name" -c \
  "pip install -q 'pytest>=8,<9' 'pytest-asyncio>=0.24,<2' && PYTHONPATH=src pytest -q"

docker run --rm --entrypoint storelink-buyer-demo \
  -e "KORRAL_STORE_KEY_47=$canary" "$image_name" >"$output_file" 2>&1

if rg -F "$canary" "$output_file" >/dev/null; then
  echo "FAIL: canary credential appeared in terminal output" >&2
  exit 1
fi

if docker image inspect "$image_name" | rg -e 'KORRAL_STORE_KEY_[0-9]+=' -e 'LEAK_TEST_.*SECRET' >/dev/null; then
  echo "FAIL: credential-like value appeared in image configuration" >&2
  exit 1
fi

if docker history --no-trunc "$image_name" | rg -e 'KORRAL_STORE_KEY_[0-9]+=' -e 'LEAK_TEST_.*SECRET' >/dev/null; then
  echo "FAIL: credential-like value appeared in image history" >&2
  exit 1
fi

if rg -n --hidden -g '!audit/**' -g '!.git/**' \
